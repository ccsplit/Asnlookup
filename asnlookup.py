...
def parse_args():
    ...
    org = parser.add_argument(
        "-o", "--org", help="Organization to look up", required=True
    )
    nmapscan = parser.add_argument(
        "-n",
        "--nmapscan",
        help="Run Nmap",
        required=False,
        action="store",
        nargs="?",
        const="-p 1-65535 -T4 -A -v",
    )
    masscan = parser.add_argument(
        "-m",
        "--masscan",
        help="Run Masscan",
        required=False,
        action="store",
        nargs="?",
        const="-p0-65535 --rate 200",
    )
    output = parser.add_argument(
        "--output", help="Output path (optional)", required=False
    )
...
def download_db():
    ...
    if (os.path.isfile("./GeoLite2-ASN-Blocks-IPv4.csv")) == False:
        ...
        os.system(
            "wget https://geolite.maxmind.com/download/geoip/database/GeoLite2-ASN-CSV.zip && unzip GeoLite2-ASN-CSV.zip && rm -f GeoLite2-ASN-CSV.zip && mv GeoLite*/* . && rm -f GeoLite2-ASN-Blocks-IPv6.csv && rm -f COPYRIGHT.txt LICENSE.txt && rm -rf GeoLite*/"
        )
        print(colored("\nDone!\n", "red"))
        ...
        try:
            ...
            print(
                colored(
                    "[*] Timed out while trying to the database server, please run the tool again.",
                    "red",
                )
            )
            sys.exit(1)
        with open("filesize.txt", "r") as filesize:
            for line in filesize:
                if line == response.headers["Content-Length"]:
                    ...
                    print(
                        colored(
                            "[*] It seems like you have not updated the database.",
                            "red",
                        )
                    )
                    try:
                    ...
                    if choice.upper() == "Y":
                        ...
                        print(
                            colored(
                                "[*] Downloading a new copy of the database ...\n",
                                "red",
                            )
                        )
                        os.system(
                            "wget -O GeoLite2-ASN-CSV.zip https://geolite.maxmind.com/download/geoip/database/GeoLite2-ASN-CSV.zip && unzip GeoLite2-ASN-CSV.zip && rm -f GeoLite2-ASN-CSV.zip && mv GeoLite*/* . && rm -f GeoLite2-ASN-Blocks-IPv6.csv  && rm -f COPYRIGHT.txt LICENSE.txt && rm -rf GeoLite*/"
                        )
...
def extract_ip(asns, organization, output_path=None):
    ...
    for asn in asns:
        try:
            ...
            print(
                colored(
                    "[*] Timed out while trying to the ASN lookup server, please run the tool again.",
                    "red",
                )
            )
            sys.exit(1)
    ...
    print(
        colored(
            "[*] IP addresses owned by {} are the following (IPv4 or IPv6):".format(
                organization
            ),
            "red",
        )
...
def scanning(n, m, organization):
    ...
    if n and m:
        print(
            colored(
                "\n[*] Please only select one port scanner: -m --> Masscan or -n --> Nmap.",
                "red",
            )
        )
    ...
    # Run Nmap on the IP addresses if -m argument is set
        ...
        if os.path.isfile("./output/" + organization + "_ipv4.txt") == True:
            ...
            os.system(
                "nmap {} -iL {}".format(n, "./output/" + organization + "_ipv4.txt")
            )
            ...
            if os.path.isfile("./output/" + organization + "_ipv6.txt") == True:
                ...
                os.system(
                    "nmap {} -iL {}".format(n, "./output/" + organization + "_ipv6.txt")
                )
        else:
            pass
    ...
    # Run Masscan on the IP addresses if -m argument is set
        ...
        if os.path.isfile("./output/" + organization + "_ipv4.txt") == True:
            print(
                colored(
                    "\n[*] Running port scanning using Masscan (Warning: supports only IPV4)...",
                    "red",
                )
            )
            os.system(
                "masscan {} -iL {}".format(m, "./output/" + organization + "_ipv4.txt")
            )
    else:
        pass
